 ...
 * since the Flash runtime virtual machine for ActionScript 3.0 (AVM2) started.
 * For a Flash runtime processing ActionScript 2.0, this method returns
 * the number of milliseconds since the Flash runtime began initialization.
 * Flash runtimes use two
 * virtual machines to process ActionScript.
 ...
 * The getTimer() method behavior for AVM1 is different than the
 * behavior for AVM2.
 * For a calendar date (timestamp), see the Date object.
 * @return	The number of milliseconds since the runtime was initialized
 * (while processing ActionScript 2.0), or since the virtual machine started (while
