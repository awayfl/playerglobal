 ...
 * <codeph class="+ topic/ph pr-d/codeph ">readExternal()</codeph>
 ...
 * These methods must be symmetrical with
 * the supertype to save its state.
 * These methods supercede the native Action Message Format (AMF) serialization behavior.
 * <p class="- topic/p ">If a class does not implement, nor inherits from a class which implements,
 * the IExternalizable interface, then an instance
 * of the class will be serialized using the default mechanism of public members only.
 * As a result, private, internal, and
 * protected members of a class will not be available.</p><p class="- topic/p ">
