...
const forbiddenNames = ['send', 'connect', 'close', 'allowDomain', 'allowInsecureDomain', 'client',
	'domain'];
...
export class LocalConnection extends EventDispatcher
	...
	constructor () {
		...
		const url = new (<any>URL)(getCurrentABC().env.url);
	...
	public handleMessage(methodName: string, argsBuffer: ArrayBuffer): void {
		...
		const errorEvent = new AsyncErrorEvent('asyncError', false, false,
