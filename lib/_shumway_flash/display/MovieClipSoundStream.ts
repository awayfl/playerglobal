...
class MediaSourceStreamAdapter extends HTMLAudioElementAdapter {
	...
	private _openMediaSource() {
		const sourceBuffer = this._mediaSource.addSourceBuffer(MP3_MIME_TYPE);
...
class BlobStreamAdapter extends HTMLAudioElementAdapter {
	...
	finish() {
		const blob = new Blob(this._rawFrames);
...
function syncTime(element, movieClip) {
	...
	element.addEventListener('timeupdate', function (e) {
		...
		const mediaDelta = element.currentTime - startMediaTime;
		const realDelta = performance.now() - startRealTime;
...
class WebAudioAdapter implements ISoundStreamAdapter {
	...
	finish() {
		...
		const sound = Sound.initializeFromPCMData(this._sec, this._data);
...
export class MovieClipSoundStream {
	...
	public constructor(streamInfo: SoundStream, movieClip: MovieClip) {
		...
		const sec = movieClip.sec;
		const isMP3 = streamInfo.format === 'mp3';
		/*
    ...
    */
		...
		const totalSamples = (streamInfo.samplesCount + 1) * this.movieClip.totalFrames * streamInfo.channels;
	...
	public playFrame(frameNum: number) {
		...
		const PAUSE_WHEN_OF_SYNC_GREATER = 1.0;
		const PLAYBACK_ADJUSTMENT = 0.25;
        ...
        !isNaN(this.soundStreamAdapter.currentTime)) {
			...
			const time = this.seekIndex[frameNum] /
