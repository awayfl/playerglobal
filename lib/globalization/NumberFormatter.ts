 ...
 * It can format <codeph class="+ topic/ph pr-d/codeph ">int</codeph>,
 * <codeph class="+ topic/ph pr-d/codeph ">uint</codeph>, and
 * <codeph class="+ topic/ph pr-d/codeph ">Number</codeph> objects.
 *
 ...
 * the grouping separator, the grouping pattern, and other elements within the number format
 * can vary depending on the locale.
 * </p><p class="- topic/p ">If the operating system supports the requested locale, the number formatting properties
 ...
 * <codeph class="+ topic/ph pr-d/codeph ">actualLocaleIDName</codeph> property.
 * </p><p class="- topic/p ">
...
export class NumberFormatter extends ASObject {
	 ...
	 * is constructed.When this property is assigned a value and there are no errors or warnings,
	 * the lastOperationStatus property
	 * is set to:LastOperationStatus.NO_ERROROtherwise the lastOperationStatus property
	 * is set to one of the constants defined in the
	 ...
	 * digits 0 through 9.  This property defines the set of digits to be used.
	 * The value of this property represents the Unicode value for the zero digit of a decimal digit set.
	 * The valid values for this property are defined in the NationalDigitsType class.
	 * When this property is assigned a value and there are no errors or warnings, the lastOperationStatus property
	 * is set to:LastOperationStatus.
	 * NO_ERROROtherwise the lastOperationStatus property is set to one of the constants defined in the
	 * LastOperationStatus class.
	 * @throws	TypeError if this property is assigned a null value.
	 */
	 ...
	 * varies depending on the user's operating system.
	 * When the trailingZeros property is set to true, the fractional portion of the
	 ...
	 * The following table shows examples of formatting the number 123456789.12 with various grouping patterns.
	 ...
	 * For patterns without an asterisk, some operating systems only support one number
	 * while others support up to three numbers.
	 ...
	 * When this property is set to false a leading zero is not included.
	 * For example if the number is 0.321 and this property is set true, then the leading
	 ...
	 * trailingZerosleadingZerofractionalDigits0.120truetrue30.1200.000falsetrue30.120true
	 * false3.120.000falsefalse3.120When this property is assigned a value
	 * and there are no errors or warnings, the lastOperationStatus property
	 * is set to:LastOperationStatus.NO_ERROROtherwise the lastOperationStatus property
	 * is set to one of the constants defined in the
	 * LastOperationStatus class.
	 * @throws	TypeError if this property is assigned a null value.
	 ...
	 * the negativeSymbol property and the 'n' character is replaced with the formatted numeric value.
	 ...
	 * format when formatting a number that is less than zero. It is not used in negative number formats
	 * that do not include
	 * a negative sign (e.g. when negative numbers are enclosed in parentheses).
	 * This property is set to a default value for the actual locale selected when this
	 * formatter is constructed. It can be set with a value to override the default setting.
	 * When this property is assigned a value and there are no errors or warnings, the lastOperationStatus property
	 * is set to:LastOperationStatus.NO_ERROROtherwise the lastOperationStatus property
	 * is set to one of the constants defined in the
	 * LastOperationStatus class.
	 ...
	 * When this property is set to false then no trailing zeros are shown.
	 * For example if the numeric value is 123.4, and this property is set true, and the fractionalDigits property
	 ...
	 * trailingZerosleadingZerofractionalDigits0.120truetrue30.1200.000falsetrue30.120
	 * truefalse3.120.000falsefalse3.120When this property is assigned a value
	 * and there are no errors or warnings, the lastOperationStatus property
	 * is set to:LastOperationStatus.NO_ERROROtherwise the lastOperationStatus property
	 * is set to one of the constants defined in the
	 * LastOperationStatus class.
	 ...
	 * For example: 123456789.22The symbol to be used as a grouping separator is defined
	 * by the groupingSeparator property. The number of digits
	 * between grouping separators is defined by the groupingPattern property.
	 * When this property is assigned a value and there are no errors or warnings, the lastOperationStatus property
	 * is set to:LastOperationStatus.NO_ERROROtherwise the lastOperationStatus property
	 * is set to one of the constants defined in the
	 * LastOperationStatus class.
	 ...
	 * If the value passed in is too large or small, such as a value greater than 1.72e308 or
	 * less than 1.72e-308, then this function returns 0.
	 *
	 *   When this method is called and it completes successfully,
	 * the lastOperationStatus property is set to:
	 * LastOperationStatus.
	 * NO_ERROROtherwise the lastOperationStatus property is set to one of the constants defined in the
	 * LastOperationStatus class.
	 ...
	 * the name of the fallback locale ID. To format based on the user's current operating system preferences,
	 * pass the value LocaleID.DEFAULT
	 ...
	 * the value of the first number found in the input string,
	 * the starting index for the number within the string, and the index
	 * of the first character after the number in the string.
	 * If the string does not contain a number, the value property of the NumberParseResult is set to NaN and the
	 ...
	 * digits, and the groupingSeparator property to determine
	 * which characters are allowed within the digits of a number,
	 * and the negativeNumberFormat property to control how negative values are represented.
	 * The following table identifies the result of strings parsed for the various NegativeNumberFormat
	 * values:NegativeNumberFormatInput
	 * StringResult(n)"(123)" or "( 123 )""-123"-n"-123" or "- 123""-123"- n"-123" or "- 123""-123"n-"123-"
	 * or "123 -""-123"n -"123-" or "123 -""-123"
	 ...
	 * is set to:LastOperationStatus.NO_ERROR
	 * Otherwise the lastOperationStatus property is set to one of the constants defined in the
	 * LastOperationStatus class.
	 ...
	 * character is a character that has a Space Separator (Zs) property in the Unicode Character Database
	...
	private ctor (requestedLocaleIDName: string): void {
