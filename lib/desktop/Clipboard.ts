 ...
 * The operating system clipboard can be accessed through the static
 * <codeph class="+ topic/ph pr-d/codeph ">generalClipboard</codeph> property.
 *
 ...
 * with the <codeph class="+ topic/ph pr-d/codeph ">setData()</codeph>
 * or <codeph class="+ topic/ph pr-d/codeph ">setDataHandler()</codeph> method.
 * </p><p class="- topic/p ">The standard formats are:</p><ul class="- topic/ul ">
 * <li class="- topic/li ">BITMAP_FORMAT: a BitmapData object (AIR only)</li><li class="- topic/li ">
 * FILE_LIST_FORMAT: an array of File objects (AIR only)</li>
 * <li class="- topic/li ">HTML_FORMAT: HTML-formatted string data</li><li class="- topic/li ">
 * TEXT_FORMAT: string data</li><li class="- topic/li ">RICH_TEXT_FORMAT:
 * a ByteArray containing Rich Text Format data</li><li class="- topic/li ">
 * URL_FORMAT: a URL string (AIR only)</li></ul><p class="- topic/p ">
 * These constants for the names of the standard formats are defined in the ClipboardFormats class.</p>
 ...
 * originating application.</p><p class="- topic/p ">
 ...
 * codeph class="+ topic/ph pr-d/codeph ">setDataHandler()</codeph> method.
 ...
 * in an undesireable pause as the file data is downloaded or created.</p><p class="- topic/p ">
 * <b class="+ topic/ph hi-d/b ">Note for AIR applications:</b>
 * The clipboard object referenced by the event objects dispatched for HTML
 * drag-and-drop and copy-and-paste events are not the same type as the AIR Clipboard object.
 * The JavaScript clipboard object is described in the AIR developer's guide.</p><p class="- topic/p ">
 * <b class="+ topic/ph hi-d/b ">
 * Note for Flash Player applications:</b> In Flash Player 10, a paste operation from the clipboard first
 ...
 * and is acting on a paste event. Calling <codeph class="+ topic/ph pr-d/codeph ">
 * Clipboard.getData()</codeph> under any other circumstances
 * will be unsuccessful. The same restriction applies in AIR for content outside the
 * application sandbox.</p><p class="- topic/p ">
 * On Linux, clipboard data does not persist when an AIR application closes.</p>
 */
export class Clipboard extends ASObject {
	 ...
	 * You should always clear the clipboard before writing new data to it
	 * to ensure that old data in all formats is erased.
	 * The generalClipboard object cannot be passed to the AIR NativeDragManager.
	 ...
	 * objects can be used for only one drag-and-drop gesture;
	 * they cannot be reused.Do not create a Clipboard object for copy-and-paste operations. Use the single
	 * Clipboard.generalClipboard object instead.
	 * @internal	the example located at examples\Clipboard.clipboard.1.as
	 * should not be displayed with FP10 docs because FP10 will console.log an error when new Clipboard() is called.
	 * @throws	IllegalOperationError new Clipboard() is not supported in Flash Player,
	 * since only the operating system clipboard can be used in Flash Player.
	 * For copy-and-paste operations involving the operating system clipboard,
	 * use the Clipboard.generalClipboard object rather than creating a new Clipboard object.
	 * Does not console.log an error in an AIR application.
	 */
	constructor () {
		super();
	}
	 ...
	 * as a URL, and as an encoded PNG file for transfer to the native file system.
	 * The data parameter must be the appropriate data type for the specified
	 * format:FormatTypeDescriptionClipboardFormats.TEXT_FORMATStringstring dataClipboardFormats.
	 * HTML_FORMATStringHTML string dataClipboardFormats.URL_FORMATStringURL string (AIR only)
	 * ClipboardFormats.RICH_TEXT_FORMATByteArrayRich Text Format dataClipboardFormats.
	 * BITMAP_FORMATBitmapDatabitmap data (AIR only)ClipboardFormats.
	 * FILE_LIST_FORMATarray of Filean array of files (AIR only)Custom format name
	 * bject reference and serialized cloneCustom format names cannot begin with "air:" or "flash:".
	 * To prevent name collisions
	 * when using custom formats, you may want to use your application ID or a package name as a
	 * prefix to the format, such as "com.example.applicationName.dataPacket".
	 * When transferring within or between applications, the serializable parameter
	 ...
	 * when accessing the data for that format.Note:
	 ...
	 * parameter does not affect the availability of data in the standard formats to non-Flash-based applications.
	 * To defer rendering of the data for a format, use the setDataHandler() method instead.
	 ...
	 * Note: On Mac OS, when you set the format parameter to ClipboardFormats.URL_FORMAT,
	 ...
	 * FormatReturn TypeClipboardFormats.TEXT_FORMATStringClipboardFormats.HTML_FORMAT
	 * StringClipboardFormats.URL_FORMATString (AIR only)ClipboardFormats.RICH_TEXT_FORMAT
	 * ByteArrayClipboardFormats.BITMAP_FORMATBitmapData (AIR only)ClipboardFormats.
	 * FILE_LIST_FORMATArray of File (AIR only)ClipboardFormats.
	 * FILE_PROMISE_LIST_FORMATArray of File (AIR only)Custom format nameNon-void
	 * The handler function is called when and only when the data in the specified format is read.
	 * Note that in some cases,
	 * the operating system calls the function before a drop occurs. For example, when you use a handler function to
	 * provide the data for a file dragged from an AIR application to the file system, the operating
	 * system calls the data handler function as soon as the drag gesture leaves the AIR application—typically resulting
	 * in an undesireable pause as the file data is downloaded or created.
	 * You can use a URLFilePromise for this purpose instead.
	 ...
	 * function again. Do not rely on either behavior.
	 * In the application sandbox of Adobe AIR, setDataHandler() can be called anytime. In other
	 ...
	 * format name, then the handler function is never called.Note:
	 * On Mac OS, when you set the format parameter to ClipboardFormats.URL_FORMAT,
	 * the URL is transferred only if the handler function returns a valid URL.
	 * Otherwise, the Clipboard object is emptied
